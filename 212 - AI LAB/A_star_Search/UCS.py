...
class UCS:
    def build_adj_list(self, file_name):
        with open(file_name, 'r') as f:
            ...
            self.nodes, self.edge, self.goalnode = [int(eachval) for eachval in firstline.split()]
            print(self.nodes, self.edge, self.goalnode)

            self.adj_list = [[] for node in range(0, self.nodes + 1)]

            for eachline in range(self.edge):
                curline = f.readline()
                leftnode, rightnode, weight = [int(eachval) for eachval in curline.split()]

                self.adj_list[leftnode].append([rightnode, weight])
                # self.adj_list[rightnode].append([leftnode, weight])  # for directed graph, skip this line

            for node in range(1, self.nodes + 1):
                print(node, self.adj_list[node])

    def main_ucs(self, src_node):  # past work, track cost and parent, node object
        ...
        while not pq.empty():
            ...
            if popped_node_number in E:
                ...
                neighbors_list = self.adj_list[popped_node_number]  #[[2,9], [3,1], ... ],  so when A* [[[2,6],[2,8]],[[2,1]]] etc
